@extends('layout')
@section('content')
    <h1 class="text-center text-primary fw-bold">Edit Transaction</h1>
    ...
    <form method="POST" action="{{ route('transaction.update', $transaction) }}">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="type" id="type" onchange="toggleFields()">
                ...
                <option value="credit" {{ old('type', $transaction->type) == 'credit' ? 'selected' : '' }}>Credit (Money Received)</option>
                <option value="cash_withdrawal" {{ old('type', $transaction->type) == 'cash_withdrawal' ? 'selected' : '' }}>Cash Withdrawal</option>
                <option value="bank_transfer" {{ old('type', $transaction->type) == 'bank_transfer' ? 'selected' : '' }}>Bank Transfer</option>
            </select>
            @error('type')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        ...
        <div class="mb-3" id="party_name_field" style="display: none;">
            ...
            <input type="text" class="form-control" name="party_name" id="party_name"
                value="{{ old('party_name', $transaction->party_name) }}">
            @error('party_name')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-3" id="transfer_to_bank_field" style="display: none;">
            ...
            <select class="form-select" name="transfer_to_bank_id" id="transfer_to_bank_id">
                <option value="">-- Select Bank --</option>
                @foreach ($banks as $bank)
                    <option value="{{ $bank->id }}" {{ old('transfer_to_bank_id', $transaction->transfer_to_bank_id) == $bank->id ? 'selected' : '' }}>
                        {{ $bank->name }} ({{ $bank->account_title }})
                    </option>
                @endforeach
            </select>
    ...
    <script>
        function toggleFields() {
            ...
            document.getElementById('party_name_field').style.display = type === 'credit' ? 'block' : 'none';
            document.getElementById('transfer_to_bank_field').style.display = type === 'bank_transfer' ? 'block' : 'none';
        }
